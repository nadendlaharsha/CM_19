...
import pandas as pd
from backend.engagement_fusion import get_engagement_fusion_summary
from backend.multimodal_fusion import get_multimodal_fusion_summary
...
# Configure Google Generative AI with API key rotation support
...
def call_gemini_safe(prompt, max_retries=None, base_delay=10):
    """
    Call Gemini API with automatic rotation between multiple keys when rate limited (429)
    """
    import time
    
    # Initialize index if not exists in session state
    if 'current_key_index' not in st.session_state:
        st.session_state['current_key_index'] = 0
        
    num_keys = len(GOOGLE_API_KEYS)
    if num_keys == 0:
        st.error("❌ No Google API Keys found. Please check your .env file.")
        return None
        
    # If max_retries not specified, attempt 2 full cycles through all keys
    actual_retries = max_retries if max_retries is not None else (num_keys * 2)

    for attempt in range(actual_retries):
        try:
            # Use current key index
            idx = st.session_state['current_key_index'] % num_keys
            current_key = GOOGLE_API_KEYS[idx]
            
            # Re-configure with current active key
            ...
            if "429" in error_str:
                ...
                new_idx = st.session_state['current_key_index'] % num_keys
                
                # Notification in UI
                st.info(f"🔄 Rate limit hit for Key {idx+1}. Rotating to Key {new_idx+1}...")
                # Small mandatory sleep to avoid burst-fire across keys
                time.sleep(2)
                
                # If we've cycled through all keys in this attempt block, apply exponential backoff
                if (attempt + 1) % num_keys == 0:
                    wait_time = base_delay * (2 ** (attempt // num_keys))
                    st.warning(f"⏳ All keys currently rate limited. Waiting {wait_time}s...")
                    time.sleep(wait_time)
            else:
                st.error(f"❌ Gemini API Error: {e}")
                return None
                
    st.error("❌ API Quota Exceeded across all provided keys. Please try again later.")
    return None
...
def refine_batched_textrank_summaries(raw_keyframes):
    """
    Refine multiple raw TextRank summaries in a single Gemini call to save API rate limits.
    """
    ...
    try:
        prompt = """
        Refine the following extractive summaries into professional, grammatically correct, and well-structured summaries.
        
        RULES:
        1. Fix any grammar, punctuation, or capitalization issues.
        2. Maintain all key facts from the original text.
        3. Keep the response concise but comprehensive. Ensure a logical flow.
        4. Output the refined summaries exactly in the format provided below, maintaining the EXACT SAME [SEGMENT X] delimiters.
        
        Format to output:
        [SEGMENT 0]
        <refined summary for segment 0>
        [SEGMENT 1]
        <refined summary for segment 1>
        ...
        
        Original Summaries to refine:
        """
        
        for i, kf in enumerate(raw_keyframes):
            prompt += f"\n[SEGMENT {i}]\n{kf.get('raw_summary', '')}\n"
            
        refined_text = call_gemini_safe(prompt)
        
        if not refined_text:
            # Fallback to raw summaries if API fails
            for kf in raw_keyframes:
                kf['summary'] = kf.get('raw_summary', '')
            return raw_keyframes
            
        # Parse the response using regex to find [SEGMENT i] blocks
        for i, kf in enumerate(raw_keyframes):
            pattern = rf"\[SEGMENT {i}\](.*?)(?:\[SEGMENT \d+\]|$)"
            match = re.search(pattern, refined_text, re.DOTALL | re.IGNORECASE)
            
            if match and match.group(1).strip():
                # remove any extra empty lines around it
                refined_segment = match.group(1).strip()
                kf['summary'] = refined_segment
            else:
                kf['summary'] = kf.get('raw_summary', '')
                
        return raw_keyframes
    except Exception as e:
        print(f"Batched refinement error: {e}")
        for kf in raw_keyframes:
            if 'summary' not in kf:
                kf['summary'] = kf.get('raw_summary', '')
        return raw_keyframes
...
def extract_keyframes_with_textrank(transcript_with_timestamps, num_keyframes=7):
    ...
    try:
        ...
        # Batch refine all keyframes in a single API call to save rate limit
        keyframes = refine_batched_textrank_summaries(keyframes)
...
def show_login_page():
    ...
    st.title("🎥 FUSION BASED VIDEO SUMMARIZATION")
    st.markdown("### INTEGRATING TRANSCRIPTS AND KEYFRAMES FOR YOUTUBE CONTENT ANALYSIS")
    
    # Check if database is configured
    if not DB_PATH:
        st.warning("⚠️ Database not configured. Authentication disabled.")
        st.info("To enable login, please ensure DB_PATH is set.")
...
def show_main_app():
                ...
                if user_data:
                    ...
                    if not history:
                        ...
                        for item in history:
                            ...
                            label = f"{item.get('video_title', 'Video')} ({str(item.get('created_at'))[:16]})"
            ...
            if st.session_state['transcript_text'] or "Multimodal" in summarization_method:
                ...
                if ("TextRank" in summarization_method or "Hybrid" in summarization_method) and (st.session_state['engagement_df'] is None or st.session_state['engagement_result'] is None) and transcript_with_timestamps:
                     with st.spinner("📊 Fusing Engagement Highlights..."):
                        ...
                        # Batch-refine all highlights in ONE Gemini call
                        long_highlights = [h for h in highlights if len(h['text'].split()) > 15]
                        if long_highlights:
                            for h in long_highlights:
                                h['raw_summary'] = textrank_summarize(h['text'], num_sentences=4, use_pca=False, min_words=30)
                            refined = refine_batched_textrank_summaries(long_highlights)
                            for h in refined:
                                h['text'] = h.get('summary', h.get('raw_summary', h['text']))
    ...
    with st.expander("🛠️ Troubleshooting"):
        st.info("If you see errors, check your .env file for GOOGLE_API_KEY.")
...
def main():
    st.set_page_config(
        page_title="🎥 FUSION BASED VIDEO SUMMARIZATION: INTEGRATING TRANSCRIPTS AND KEYFRAMES FOR YOUTUBE CONTENT ANALYSIS", 
        layout="wide",
        page_icon="🎥"
    )
